...
def _search_arxiv(query: str, limit: int) -> List[Dict]:
    ...
    # In arxiv 2.x this is an iterator of arxiv.Result
...
def _download_pdf_bytes(entry) -> bytes:
    ...
    try:
        ...
        if resp.status_code == 200 and resp.content:
            return resp.content
        logger.warning(f"PDF HTTP {resp.status_code} for {getattr(entry, 'title', 'unknown')}")
        return b""
...
def fetch_and_analyze_arxiv(topic: str):
    ...
    combined = "\n\n".join(
        s["summary"] for s in per_paper_summaries if s.get("summary")
    )
    final = (
        call_llm(
